...
namespace Handler {
	...
	{
		std::ifstream input_file{ input_argument };
		...
		while (std::getline(input_file, curr_line)) {
			...
			{
				apartment_list = apartment_list->remove_apartment(splitted_line[1]);
			}
			else if (splitted_line[0].compare("make_flat_empty")==0) 
			{
			...
			{
				apartment_list = apartment_list->merge_two_apartments(splitted_line[1], splitted_line[2]);
			}
			...
			{
				if (apartment_list == NULL)
				{
					output_file << "There is no apartment in the street.";
				}
				else 
				{
