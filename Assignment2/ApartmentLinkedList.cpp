...
void ApartmentLinkedList::add_apartment(ApartmentNode* new_node, std::string command)
{
	...
	Handler::split(splitted_command, command, '_');

	std::string insert_command = splitted_command[0];
	std::string insert_apartment_name = splitted_command[1];
	
	if (insert_command.compare("after")==0)
	{
		insert_after(insert_apartment_name,new_node);
	}
	else if  (insert_command.compare("before")==0)
	{
		insert_before(insert_apartment_name,new_node);
	}
	num_of_apartments += 1;
}
...
void ApartmentLinkedList::add_flat(std::string apartment_name, int index,int initial_bandwidth, int id)
{
	...
	{
		FlatNode* new_node = FlatNode::getnode(id, curr->max_bandwidth - flat_list->sum_of_initial_bandwidths());
		flat_list->add_to_index(index, new_node);
	}
	else
	{
...
int ApartmentLinkedList::find_sum_of_max_bandwidth()
{
	...
	{
		ApartmentNode* curr = head;
		for (int i = 0; i<num_of_apartments+1 ;i++)
		{
...
void ApartmentLinkedList::insert_head(ApartmentNode* new_node)
{
	head = new_node;
	tail = new_node;
	tail->nextP = head;
...
void ApartmentLinkedList::insert_before(std::string insert_apartment_name, ApartmentNode* new_node)
{
	...
	else
	{
        ...
        new_node->nextP = prev->nextP;
        prev->nextP = new_node;
	}
}

void ApartmentLinkedList::insert_after(std::string insert_apartment_name, ApartmentNode* new_node)
{
